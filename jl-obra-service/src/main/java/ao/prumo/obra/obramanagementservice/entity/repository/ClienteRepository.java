...
{
    @Query("SELECT c FROM Cliente c WHERE c.organizacaoId.id = :organizacaoId AND c.status IS TRUE")
    Page<Cliente> findByOrganizacaoId(@Param("organizacaoId") UUID organizacaoId, Pageable pageable);

    @Query("SELECT c FROM Cliente c WHERE c.id = :id AND c.organizacaoId.id = :organizacaoId  AND c.status IS TRUE")
